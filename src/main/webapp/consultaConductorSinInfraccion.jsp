<%-- 
    Document   : consultaConductorSinInfraccion
    Created on : 19 nov. 2020, 17:42:20
    Author     : JUAN MORENO
--%>

<%@page import="com.mysql.cj.protocol.Resultset"%>
<%@page import="com.mysql.jdbc.Driver"%>
<%@page import="java.sql.Statement"%>
<%@page import="java.sql.*"%>
<%@page import="java.sql.Connection"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" integrity="sha384-wvfXpqpZZVQGK6TAh5PVlGOfQNHSoD2xbE+QkPxCAFlNEevoEH3Sl0sibVcOQVnN" crossorigin="anonymous">
        <link href="Productos/estilos.css" rel="stylesheet" type="text/css"/>
        <title>Consultar Conductores Sin Infracciones</title>
    </head>
   <body  style="background-image: url('https://besthqwallpapers.com/img/original/99458/blue-creative-texture-dark-blue-abstract-background-creative-blue-backgrounds-background-with-lines-stylish-blue-texture.jpg');">

        <%
            Connection con = null;
            Statement st = null;
            ResultSet rs = null;

            
        %>

        
            <form  class="datos" class="form-group">
                 <h1 style="text-align: center;" class="container mt-5 ">Consultar Conductores Sin Infracciones</h1>

        <div class="container mt-5 ">
                <div class="form-group">
                    <label for="cedula">Buscar por Cedula</label>
                    <input type="text" name="cedulaBuscar" class="form-control mr-sm-2"  id="cedulaBuscar" placeholder="cedulaBuscar" >
                </div>

                <button type="submit" name="buscar" class="btn btn-primary">Buscar</button>
           

        </div>
        <table class="table table-bordered container mt-5">
            <thead class="thead-dark">
                <tr>
                    <th scope="col">Id Conductor</th>
                    <th scope="col">Nombre</th>
                    <th scope="col">Telefono</th>
                    <th scope="col">Licencia</th>
                    <th scope="col">ID Vehiculo</th>
                    <th scope="col">Puntos</th>
                </tr>
            </thead>
            <tbody class="table-light">
                <%            
                    if (request.getParameter("buscar") != null) {
                    String cedulaBuscar = request.getParameter("cedulaBuscar");
                        try {
                        System.out.println("pasa");
                            String url = "jdbc:mysql://localhost:3306/software";
                            String username = "root";
                            String password = "";
                            Class.forName("com.mysql.jdbc.Driver");

                            con = DriverManager.getConnection(url, username, password);
                            st = con.createStatement();
                            rs = st.executeQuery("SELECT c.id_conductor, c.nombre, c.telefono, c.licencia, c.id_vehiculo, c.puntos FROM conductor c WHERE c.id_conductor="+cedulaBuscar+ " AND NOT EXISTS (SELECT NULL FROM multa m WHERE m.id_conductor ="+cedulaBuscar +")");
                          
                            while (rs.next()) {
                            

                %>
                <tr>
                    <th scope="row"> <%= rs.getInt(1)%> </th>
                    <td><%= rs.getString(2)%></td>
                    <td><%= rs.getString(3) %></td>
                    <td><%= rs.getString(4)%></td>
                    <td><%= rs.getInt(5)%></td>
                    <td><%= rs.getInt(6)%></td>
                </tr>
                <%
                    
                    }
                }catch (Exception e) {%>
            <div class="alert alert-danger" role="alert"><%
                        out.print("Alerta: Campos vacios");
                    }
                }
                %></div>
        </tbody>
    </table>    
            </form>
        <div class="container mt-5 ">
        <button type="submit" class="btn btn-light "><a href="inicioDti.jsp"><i class="fa fa-reply" aria-hidden="true">Regresar Pagina Principal</i></a></button>
    </div>
               
</body>
</html>
